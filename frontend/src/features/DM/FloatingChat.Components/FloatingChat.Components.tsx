...
import {
  LuSearch,LuCheck,
  LuCheckCheck,LuImage,LuSmile,LuLoader,
  LuSend,LuShoppingBag,LuTruck,LuCheckCheck as LuCheckCircle,
  LuStar,LuCircleAlert,LuMessageCircle,
      ...
      >
        <div className="flex items-center gap-2 overflow-hidden">
          ...
          {view === "chat" && activeChatInfo ? (
            ...
            <h3 className="font-bold text-slate-800 text-sm ml-1">
              Mensajes
            </h3>
          )}
        </div>
  ...
  return (
    <div className="px-2 py-2 bg-white border-t border-slate-100 relative shrink-0">
      ...
      <form
        ...
        className="flex items-end gap-1.5 bg-slate-50 p-1.5 rounded-[20px] border border-slate-100 focus-within:border-blue-200 focus-within:bg-white focus-within:ring-2 focus-within:ring-blue-100/50 transition-all"
      >
        ...
        <div className="flex-1 min-w-0 py-1">
          <textarea
            ...
            rows={1}
            style={{
              overflowY: text.length > 30 ? "auto" : "hidden",
            }}
          />
        </div>
        ...
        >
          {isLoading ? (
            ...
            <LuSend size={14} className="ml-0.5" />
          )}
        </button>
      </form>
    </div>
...
// Reexportamos TransactionStatusBar y TransactionCarousel para el flotante
export {
  TransactionStatusBar,
  TransactionCarousel,
} from "../Chat.Components/Chat.Components";

// Lista de chats compacta para flotante
  ...
  <div className="flex flex-col h-full">
    <div className="px-3 py-2 bg-white sticky top-0 z-10 border-b border-slate-50">
      <div className="relative bg-slate-50 rounded-lg flex items-center px-2 h-8">
        ...
        <input
          placeholder="Buscar..."
          className="bg-transparent text-xs w-full focus:outline-none placeholder:text-slate-400 text-slate-700"
        />
      </div>
    </div>
    <div className="flex-1 overflow-y-auto custom-scrollbar">
      {chats.length === 0 ? (
        <div className="h-32 flex flex-col items-center justify-center text-slate-400 text-xs">
          <LuMessageCircle size={24} className="mb-2 opacity-20" /> No
          hay chats
        </div>
      ) : (
        chats.map((chat: Chat) => (
          <div
            ...
            className="flex items-center gap-3 p-3 hover:bg-slate-50 cursor-pointer transition-colors border-b border-slate-50/50 last:border-0"
          >
      ...
      {hasNextPage && (
        <button
          ...
          className="w-full py-2 text-[10px] text-blue-500 hover:bg-blue-50 font-medium"
        >
...
// Área de mensajes compacta
export const FloatingMessagesArea = ({
  mensajes,
  currentUserId,
}: {
  mensajes: Mensaje[];
    ...
    <div className="flex-1 overflow-y-auto p-3 space-y-2 bg-[#F8F9FC]">
      {mensajes.map((msg) => {
        if (msg.tipo === "sistema")
          return (
            <div key={msg.id} className="text-center my-2">
              <span className="text-[9px] bg-slate-100 text-slate-400 px-2 py-0.5 rounded-full border border-slate-200">
                {msg.texto}
              </span>
            </div>
          );
        ...
        return (
          ...
          >
            <div
              className={`max-w-[85%] px-3 py-1.5 rounded-2xl text-xs shadow-sm break-words ${
                isMe
            ...
            >
              {msg.imagenUrl && (
                <img
                  src={msg.imagenUrl}
                  className="rounded-lg mb-1 max-h-32 object-cover cursor-pointer border border-white/10"
              ...
              <div
                className={`text-[8px] text-right mt-0.5 ${
                  isMe ? "text-blue-100/80" : "text-slate-300"
                } flex justify-end gap-0.5 items-center`}
              >
                {msg.hora}{" "}
                {isMe &&
                  (msg.estado === "leido" ? (
                    <LuCheckCheck size={10} />
                  ) : (
                    <LuCheck size={10} />
                  ))}
              </div>
